...
export async function runLoop(providerArg, modelArg, bootContent) {
  // ============================================================
  // 1. INPUT STACK — one readline, prompt owned by readline.
  // The visible prompt is ALWAYS live; typed input is never lost:
  //   - At the main prompt: lines resolve ask() directly.
  //   - While working (streaming/tools): lines land in lineQueue as
  //     steering and are delivered at safe checkpoints. The prompt
  //     stays usable the whole time — no pause, no disappearing box.
  // ============================================================
  ...
  const lineQueue = [];
  let lineResolver = null;   // set while ask() is waiting
  let idle = true;            // at main prompt vs working
  ...
  rl.on('line', (line) => {
    if (lineResolver) {
      ...
      lineQueue.push(line);   // steering while the agent works
    }
  });
  ...
  // ask(): print optional lead-in (e.g. status line) then show the prompt.
  // While working, ask() is NOT called — the prompt stays up and typed
  // lines buffer into lineQueue via the 'line' handler above.
  const ask = (q) => {
  ...
  // Typing just "/" + Enter opens the full arrow-key command list
  // (the same proven selector used for /session and /model list).
  // Typing a partial like "/lo" + Enter runs it via prefix matching.
  // Tab gives native readline completion while typing. No live redraw
  // of the screen — that approach corrupted the terminal (deleted
  // output, mangled DELETE key). This one never touches readline's
  ...
  while (true) {
    idle = true;
    // STATUS LINE printed once, then readline shows its own '> ' prompt.
    // No manual '> ' in the ask() string — that double-prompted.
